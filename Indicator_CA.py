...
if __name__ == '__main__':
    ...
    check_path = 'Weight/checkpoints_G_Res_five_block_find_fault_itea_CA12.pth'
    ...
    for i in li_snow:
        ...
        with torch.no_grad():
            for data in data_loader:
                ...
                snow_fir_img = con_snow
                snow_sec_img = con_snow
                snow_thr_img = con_snow

                out = Seg_Mask_25(snow_img)
                Mask = torch.argmax(out, dim=1, keepdim=True)
                Mask = torch.cat([Mask, Mask, Mask], dim=1)

                Recover_snow_thr_img = snow_thr_img
                ...
                snow_sa = snow_img
                A_sa = 1-A
                save_image(1-A, "CA_image/" + i + "_result_%d.png" % number, nrow=1, normalize=False)
