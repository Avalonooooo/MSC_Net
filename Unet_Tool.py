...
class Skip(nn.Module):
    ...
    def forward(self, x):
        ...
        res = 0.85 * self.Max(out) + 0.15 * self.Avg(out)
...
class Down(nn.Module):
    ...
    def __init__(self, in_channels, out_channels):
        ...
        self.Conv1 = nn.Sequential(
            ...
            nn.GroupNorm(int(in_channels / 16), in_channels),
            #nn.ReLU(inplace=True)
        )
    ...
    def forward(self, x, fea, num, pp_fea, is_add = False):
        #print(x.shape, "!!!!!!")
        ...
        if num<3:
            if is_add == True:
                #print(out.shape, "!!!!!!", pp_fea.shape)
                out = self.ReLu(self.Conv2(out) + pp_fea)
                #print("end********************************")
            result = self.D_DC(out)
        else:
            if is_add == True:
                #print(out.shape, "!!!!!!", pp_fea.shape)
                out = self.ReLu(self.Conv1(out) + pp_fea)
                #print("end********************************")
            result = self.DC(out)
        return out, result
...
class UNet(nn.Module):
    def __init__(self, n_channels, bilinear=True):
        ...
        self.skip1_3 = Skip(n_channels + 3, 256)
        self.skip2_4 = Skip(128, 512)
        self.skip3_5 = Skip(256, 1024)
    ...
    def forward(self, x, Prex, PreFea, num):
        ...
        x1_3 = self.skip1_3(torch.cat([x, Re], dim=1))

        x_2, x2 = self.down1(x1, PreFea[0], num, [], False)
        fea.append(x2)
        x2_4 = self.skip2_4(x_2)

        x_3, x3 = self.down2(x2, PreFea[1], num, x1_3, True)
        fea.append(x3)
        x3_5 = self.skip3_5(x_3)

        x_4, x4 = self.down3(x3, PreFea[2], num, x2_4, True)
        fea.append(x4)

        x_5, x5 = self.down4(x4, PreFea[3], num, x3_5, True)
        fea.append(x5)
...
class UNet_Four(nn.Module):
    def __init__(self, n_channels, bilinear=True):
        ...
        self.skip1_3 = Skip(n_channels, 128)
        self.skip2_4 = Skip(64, 256)
        self.skip3_5 = Skip(128, 512)
        # self.outc_four = OutConv(64 + 3 + 1, 3)
    ...
    def forward(self, x, num):
        ...
        x_2, x2 = self.down1(x1, [], num, [], False)
        fea.append(x2)
        x2_4 = self.skip2_4(x_2)

        x_3, x3 = self.down2(x2, [], num, x1_3, True)
        fea.append(x3)
        x3_5 = self.skip3_5(x_3)

        x_4, x4 = self.down3(x3, [], num, x2_4, True)
        fea.append(x4)

        x_5, x5 = self.down4(x4, [], num, x3_5, True)
        fea.append(x5)
        # print (x5.shape, x4.shape)
        u = self.up1(x5, x4)
        u = self.up2(u, x3)
