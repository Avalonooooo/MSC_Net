...
def prepare_input(resolution):
    x1 = torch.FloatTensor(1, 3, 64, 64)
    x2 = torch.FloatTensor(1, 3, 64, 64)
    x3 = torch.FloatTensor(1, 3, 64, 64)
    x4 = torch.FloatTensor(1, 3, 64, 64)
    x5 = torch.FloatTensor(1, 3, 64, 64)
    x1, x2, x3, x4, x5 = x1.cuda(), x2.cuda(), x3.cuda(), x4.cuda(), x5.cuda()
    print(x1.device)
    return dict(x= x1, con_x= x2, snow_fir_img= x3, snow_sec_img= x4, Re_thr= x5)
...
if __name__ == '__main__':
    ...
    argparser.add_argument(
        ...
        default = "C:/learn/deeplearning/Desnow/cat_img",
        type=str,
    ...
    ops, params = get_model_complexity_info(net, ((1, 3, 64, 64),(1, 3, 64, 64),(1, 3, 64, 64),(1, 3, 64, 64),(1, 3, 64, 64)), input_constructor=prepare_input,as_strings=True,
    ...
    # print (check_point['min_iteration'])
    #
    ...
    #             snow_sa = snow_img
    #             A_sa = 1-A
    #             save_image(A_sa, "All_Result_test_loss/" + i + "_result_%d.png" % number, nrow=1, normalize=False)
    #             # save_image(snow_sa, "select_show_image/" + i + "_original_%d.png" % number, nrow=1, normalize=False)
    #             # save_image(Re0, "Re_image/" + i + "_Re0_%d.png" % number, nrow=1, normalize=False)
    #             # save_image(Re1, "Re_image/" + i + "_Re1_%d.png" % number, nrow=1, normalize=False)
    #             # save_image(Re2, "Re_image/" + i + "_Re2_%d.png" % number, nrow=1, normalize=False)
    #             # save_image(mer, "Result/" + i + "_result_%d.png" % number, nrow=1, normalize=False)
    #             # g = gt_img.cpu()
    #             # f = Image_1.cpu()
    #             # ps, ss = get_psnr_ssim(g,f)
    #
    #             sum_psnr += psnr(gt_img,1-A)
    #             sum_ssim += ssim(gt_img,1-A)
    #     #
    #     print (i,'sum_psnr:',sum_psnr,'sum_ssim:',sum_ssim)
    #         # temp_mask = torch.argmax(l_mask_z_o,dim = 1, keepdim = True)
    #         # l = torch.cat([temp_mask,temp_mask,temp_mask],dim=1)
    #         #
    #         # temp = torch.argmax(h_mask,dim = 1, keepdim = True)
    #         # hea_mask = torch.cat([temp,temp,temp],dim = 1)
    #         # lig_mask = torch.cat([l_mask, l_mask, l_mask],dim = 1)
    #         # hea_mask = hea_mask.to(device = args.device)
    #         # lig_mask  = lig_mask.to(device = args.device)
    #         # # print (hea_mask.shape, lig_mask.shape)
    #         # # print (final.shape, mask_h.shape, mask_l.shape)
    #         # res = torch.cat([gt_img, snow_img, final, hea_mask, lig_mask, re_l_mask,l,re,y_], dim=3)
    #         # res_next = torch.cat([mask_h, mask_l], dim = 2)
    #         # save_image(res, "result/%d.png" % number, nrow=1, normalize=False)
    #         # save_image(res_next, "result/%d_next.png" % number, nrow=1, normalize=False)
